...
class DzialaniaFizyczne():
    ...
    def wyswietl_wykres(self, czasyTab, drogiTab, timer, droga, energiePotencjalne, energieKinetyczne, predkosciTab):

        self.dane_do_wykresu(czasyTab=czasyTab, drogiTab=drogiTab, timer=timer, droga=droga, energieKinetyczne=energieKinetyczne, energiePotencjalne=energiePotencjalne, predkosciTab=predkosciTab)

    
    def dane_do_wykresu(self, czasyTab, drogiTab, timer, droga, energiePotencjalne, energieKinetyczne, predkosciTab):
        ...
        self.figure, self.ax = plt.subplots()
        self.ax.set_title("Wykres zależności prędkości od czasu")
        plt.ylabel('Prędkość')
        plt.xlabel('Czas')
        plt.plot(czasyTab, predkosciTab)

        scat = self.ax.scatter(czasyTab[0], predkosciTab[0], c="b", label=f'Czas = {round(timer,2)}s')
        line2 = self.ax.plot(czasyTab[0], predkosciTab[0], label=f'Prędkość = {round(min(predkosciTab))} m/s')[0]
        self.ax.set(xlim=[0, max(czasyTab)+1], ylim=[0, max(predkosciTab)+5], xlabel='Czas (s)', ylabel='Prędkość (m/s)')
        self.ax.legend()
