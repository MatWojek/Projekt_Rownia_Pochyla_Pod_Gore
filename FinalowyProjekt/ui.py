...
class GUI(QWidget):
    ...
    def interfejs(self):
        ...
        uklad.addWidget(self.przyspieszenieWynik)
        uklad.addWidget(self.drogaWynik)
        uklad.addWidget(self.czasWynik)
        uklad.addWidget(self.maksymalna_wysokoscWynik)
    ...
    def obliczanie(self):
        ...
        try:
            ...
            przyspieszenieWynik = f"Przyspieszenie: {DF.przyspieszenieF(wspolczynnik_tarcia=wspolczynnik_tarcia, kat_nachylenia=kat_nachylenia)}"
            czasWynik = f"Czas: {DF.czasF(predkosc_poczatkowa=predkosc_poczatkowa, kat_nachylenia=kat_nachylenia, wpolczynnik_tarcia=wspolczynnik_tarcia)}"
            drogaWynik = f"Przebyta droga: {DF.drogaF(predkosc_poczatkowa=predkosc_poczatkowa, czas=self.czasW, przyspieszenie=self.przyspieszenieW, wpolczynnik_tarcia=wspolczynnik_tarcia)}"
            maksymalna_wysokoscWynik = f"Maksymalna wysokość: {DF.maksymalna_wysokoscF(predkosc_poczatkowa=predkosc_poczatkowa, kat_nachylenia=kat_nachylenia, wpolczynnik_tarcia=wspolczynnik_tarcia)}"
